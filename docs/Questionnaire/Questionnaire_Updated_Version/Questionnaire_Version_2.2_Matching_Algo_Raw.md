...
Hard Filtering — remove fundamentally incompatible pairs
Similarity Calculation — question-level compatibility ∈ [0,1]
Importance Weighting — user-defined preference strength
Directional Scoring — how well B satisfies A
Section Weighting — lifestyle > personality
Pair Score Construction — penalize asymmetry
Eligibility Thresholding — allow "no match" outcomes
Global Matching — maximize total compatibility
...
User B's answer ≠ User A's required answer
...
If User A specifies "same" as preference: User B's answer ≠ User A's answer
...
[MAJOR CHANGES v2.2]
For each answered question, compute a similarity score ∈ [0,1].
...
Calculation:
compatibility_score =
(|user's_receive ∩ match's_show| / 2) × 0.5 +
(|match's_receive ∩ user's_show| / 2) × 0.5
...
score_final(A → B) =
0.65 × score_S1(A → B) + 0.35 × score_S2(A → B)
Rationale
...
Compute:
pair_score =
α × min(score(A→B), score(B→A)) + (1 − α) × mean(score(A→B), score(B→A))
Where: α = 0.65
