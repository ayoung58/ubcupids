...
/**
 * Preview Cupid Assignment for Production Users
 * POST /api/admin/pair-cupids-production/preview
 *
...
export async function POST() {
  try {
    ...
    // Check if compatibility scores exist for production users
    ...
    if (scoreCount === 0) {
      return NextResponse.json(
        {
          error:
            "No compatibility scores found for production users. Run the matching algorithm for production users first.",
        },
        { status: 400 },
      );
    }
    ...
    if (cupids.length === 0) {
      return NextResponse.json(
        { error: "No production cupids available" },
        { status: 400 },
      );
    }
    ...
    if (candidates.length === 0) {
      return NextResponse.json(
        { error: "No production candidates available" },
        { status: 400 },
      );
    }
