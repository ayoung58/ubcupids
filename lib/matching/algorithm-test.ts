/**
 * Matching Algorithm for Test Users Only
 *
 * This is a wrapper around the main matching algorithm that only processes test users.
 */
...
/**
 * Load test users eligible for matching
 */
async function loadEligibleTestUsers(): Promise<UserForScoring[]> {
  ...
  for (const user of users) {
    ...
    try {
      ...
      console.error(
        `Error decrypting responses for test user ${user.id}:`,
        error
      );
    }
  }
  ...
  console.log(
    `Loaded ${eligibleUsers.length} eligible TEST users for matching`
  );
...
async function scoreAllPairs(users: UserForScoring[]): Promise<ScoredPair[]> {
  ...
  console.log(`Scoring ${pairs.length} test user pairs...`);
  ...
  console.log(
    `Found ${scoredPairs.length} eligible test user pairs above minimum score`
  );
  return scoredPairs;
}
...
function runGreedyMatching(
  ...
  console.log(
    `Generated ${matches.length / 2} mutual test user matches (${matches.length} match records)`
  );
  return matches;
}
...
async function storeCompatibilityScores(
  ...
  console.log(`Storing ${pairs.length} test user compatibility scores...`);
...
async function storeMatches(
  ...
  console.log(`Storing ${matches.length} test user matches...`);
...
/**
 * Run the complete matching algorithm for TEST USERS ONLY
 */
export async function runMatchingForTestUsers(
  batchNumber: number
): Promise<MatchingResult> {
  console.log(`\n========================================`);
  console.log(`Starting matching for TEST USERS (Batch ${batchNumber})`);
  console.log(`========================================\n`);
  ...
  if (users.length < 2) {
    console.log("Not enough test users for matching");
    return {
      batchNumber,
      algorithmMatches: [],
  ...
  console.log(`\n========================================`);
  console.log(`TEST USER Matching Complete!`);
  console.log(`========================================`);
